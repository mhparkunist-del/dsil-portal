...
--   2. SQL Editor 에 이 파일 전체를 붙여 넣고 Run
...
-- 회원가입(첫 magic link 로그인) 시 profiles 자동 생성
...
-- 과제별 집계 뷰 (처리된 건만 차감)
create or replace view public.project_budget as
select
  p.id,
  p.code,
  p.name,
  ...
  p.budget - coalesce(sum(r.amount) filter (where r.status = 'done'), 0) as remaining,
